        ...
        <div class="col-12 rounded p-4 mt-4 d-md-flex gap-5">
            ...
            <div class="col-md-8 col-sm-11">
                <div class="d-flex flex-column">
                    <div class="d-flex gap-5 align-items-center" style="text-align:center">
                        <span style="font-size: xx-large; text-align:center"><?=$profile['first_name']?> <?=$profile['last_name']?></span>
...
if($user['id']!=$profile['id'] && !checkBS($profile['id'])){
  ...
  <div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                ...
                                <li><a class="dropdown-item " href="assets/php/actions.php?block=<?=$profile['id']?>&current_job=<?=$profile['current_job']?>"><i class="bi bi-x-circle-fill"></i> Blocker</a></li>
                            </ul>
                        </div>
    <?php
}
                    ...
                    <span style="font-size: larger;" class="text-secondary"><?=$profile['current_job']?></span>
                    <?php
...
<a class="btn btn-sm btn-primary"><i class="bi bi-file-post-fill"></i> <?=count($profile_post)?> Publications</a>
<a class="btn btn-sm btn-primary <?=count($profile['followers'])<1?'disabled':''?>" data-bs-toggle="modal" data-bs-target="#follower_list"><i class="bi bi-people-fill"></i> <?=count($profile['followers'])?> Abonné(s)</a>
<a class="btn btn-sm btn-primary <?=count($profile['following'])<1?'disabled':''?>" data-bs-toggle="modal" data-bs-target="#following_list"><i class="bi bi-person-fill"></i> <?=count($profile['following'])?> Abonnement(s) </a>
...
?> 
<button class="btn btn-sm btn-danger unblockbtn" data-user-id='<?=$profile['id']?>' >Débloquer</button>

<?php
}else if(checkBlockStatus($profile['id'],$user['id'])){ ?>
    <div class="alert alert-danger" role="alert">
    <i class="bi bi-x-octagon-fill"></i> <?=$profile['username']?> Vous à bloquer !
</div>
   <?php }else if(checkFollowStatus($profile['id'])){
   ?>
<button class="btn btn-sm btn-danger unfollowbtn" data-user-id='<?=$profile['id']?>' >se désabonner</button>
   
   <?php
}else{
    ?>
<button class="btn btn-sm btn-primary followbtn" data-user-id='<?=$profile['id']?>' >suivre</button>
...
?>
        ...
        <h3 class="border-bottom">Publications</h3>
        <?php
...
if(checkBS($profile['id'])){
 ...
 <div class="alert alert-secondary text-center" role="alert">
    <i class="bi bi-x-octagon-fill"></i> vous n'êtes pas autorisé à voir ces Publications !
</div>
   <?php
    
}else if(count($profile_post)<1){
    echo "<p class='p-2 bg-white border rounded text-center my-3'>Vous n'avez aucun message</p>";
}
        ?>
...
foreach($profile_post as $post){
    ...
    ?>
            <img src="assets/images/posts/<?=$post['post_img']?>" data-bs-toggle="modal" data-bs-target="#postview<?=$post['id']?>" width="100px" max-height="50px" class="rounded" />
                    ...
                    <div class="col-md-4 col-sm-12 d-flex flex-column">
                        <div class="d-flex align-items-center p-2 <?=$post['post_text']?'':'border-bottom'?>">
                            ...
                            <div class="d-flex flex-column justify-content-start">
                                ...
                                <p style="margin:0px;" class="text-muted"><?=$profile['current_job']?></p>
                            </div>
  ...
  foreach($likes as $like){
      ...
      ?>
  <li><a class="dropdown-item" href="?u=<?=$lu['current_job']?>"><?=$lu['first_name'].' '.$lu['last_name']?> (<?=$lu['current_job']?>)</a></li>
  ...
  ?> 
                ...
                <div style="font-size:small" class="text-muted">Posté <?=show_time($post['created_at'])?> </div> 
...
if(count($comments)<1){
    ?>
<p class="p-3 text-center my-2 nce">pas de commentaires</p>
    <?php
}
...
<div class="d-flex align-items-center p-2">
                                ...
                                <div class="d-flex flex-column justify-content-start align-items-start">
                                    <h6 style="margin: 0px;"><a href="?u=<?=$cuser['current_job']?>" class="text-decoration-none text-muted"><?=$cuser['current_job']?></a> - <?=$comment['comment']?></h6>
  ...
  <div class="input-group p-2 border-top">
                            <input type="text" class="form-control rounded-0 border-0 comment-input" placeholder="dite quelque chose.."
                                aria-label="Recipient's current_job" aria-describedby="button-addon2">
                            <button class="btn btn-outline-primary rounded-0 border-0 add-comment" data-cs="comment-section<?=$post['id']?>" data-post-id="<?=$post['id']?>" type="button"
                                id="button-addon2">Publié</button>
                        </div>
                            <?php
                        }else{
                            ?>
<div class="text-center p-2">
si vous voulez commenter, suivez cet utilisateur</div>
...
foreach($profile['followers'] as $f){
    ...
    if(checkFollowStatus($f['follower_id'])){
        $fbtn = '<button class="btn btn-sm btn-danger unfollowbtn" data-user-id='.$fuser['id'].' >se désabonner</button>';
...
<div class="d-flex justify-content-between">
                    <div class="d-flex align-items-center p-2">
                        ...
                        <div>&nbsp;&nbsp;</div>
                        <div class="d-flex flex-column justify-content-center">
                            <a href='?u=<?=$fuser['current_job']?>' class="text-decoration-none text-dark"><h6 style="margin: 0px;font-size: small;"><?=$fuser['first_name']?> <?=$fuser['last_name']?></h6></a>
                            <p style="margin:0px;font-size:small" class="text-muted"><?=$fuser['current_job']?></p>
                        </div>
                    </div>
                    <div class="d-flex align-items-center">
